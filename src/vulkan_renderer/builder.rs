...
use super::{render_targets::RenderTargets, vulkan_context::{QueueFamilies, SwapchainBuilder, VulkanContext}, vulkan_interface::VulkanInterface, VulkanRenderer};
...
impl VulkanRendererBuilder {
    ...
    pub unsafe fn create_interface(mut self) -> Result<Self> {
        self.interface = VulkanInterface::new(self.context.as_ref().unwrap(),
                                              self.queue_families.unwrap())?
            .pipe(Some);
        Ok(self)
    }

    pub unsafe fn create_render_targets(mut self) -> Result<Self> {
        self.render_targets = RenderTargets::new(self.context.as_ref().unwrap(),
